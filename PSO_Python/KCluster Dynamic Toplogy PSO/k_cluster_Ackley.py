...
class PSO(object):
  ...
  def __init__(self, func, init_pos, n_particles):
    ...
    self.cluster_particles = Clustering(self.particles_pos)
  ...
  def optimize(self, maxiter):
    ...
    for _ in range(maxiter):
      lbest = 0     
      clusterGBEST =  getClusterOfAParticle(self.cluster_particles ,PSO_s.g_best , self.particle_dim )
      if clusterGBEST != None :
          lbest = getPBest(self.cluster_particles[clusterGBEST] , self.particles_pos , self.particle_dim)
...
def sphere(x):
  """
    In 3D: f(x,y,z) = x² + y² + z²
  """
  return bf.XinSheYang(dimension_nb).get_func_val(x)
...
def Clustering(X):
  ...
  segmentation = scaler.fit_transform(X)
  pca = PCA()
